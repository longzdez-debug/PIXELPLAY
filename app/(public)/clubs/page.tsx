    ...
    <main className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-24 pt-28 sm:px-6 md:px-10 md:pt-32">
        <motion.div variants={fadeUp} initial="hidden" animate="visible" className="mb-12 text-center">
          ...
          <h1 className="font-display text-4xl font-black text-white md:text-5xl">ТРИ КЛУБА <span className="text-gradient-brand">PIXEL</span></h1>
          <p className="mx-auto mt-4 max-w-2xl text-white/50">Выбери клуб, посмотри схему зала в реальном времени и займи своё место.</p>
        </motion.div>
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" className="mb-10 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {CLUBS.map((club, i) => {
            ...
            return (
              <motion.button key={club.slug} onClick={() => { setActiveSlug(club.slug); setActiveZone(club.zones[0].name); }} aria-pressed={isActive} variants={staggerItem} custom={i} whileHover={{ y: -4 }} whileTap={{ scale: 0.98 }} className={`cyber-panel p-5 text-left transition-all ${isActive ? "!border-brand shadow-[0_0_24px_rgba(255,106,0,0.2)]" : ""}`}>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {isActive && <motion.span layoutId="club-active" className="h-2 w-2 rounded-full bg-brand shadow-[0_0_10px_rgba(255,106,0,0.8)]" />}
                ...
                <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-3"><span className="text-xs text-white/40">{totalPcCount(club)} ПК в зале</span></div>
              </motion.button>
            );
          })}
        </motion.div>

        <motion.div key={active.slug} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-8 flex flex-wrap gap-2">
          {active.features.map((f) => <span key={f} className="border border-brand/30 bg-brand/5 px-3 py-1.5 text-xs text-brand-light">{f}</span>)}
          <span className="border border-white/10 px-3 py-1.5 text-xs text-white/50">☎ {active.phone}</span>
        </motion.div>

        <motion.div key={active.slug + "-zones"} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mx-auto max-w-5xl">
          <div className="mb-6 flex flex-wrap items-center gap-2">
            {active.zones.map((zone) => {
              const isActiveZone = activeZone === zone.name;
              return <button key={zone.name} onClick={() => setActiveZone(zone.name)} type="button" aria-pressed={isActiveZone} className={`min-h-11 px-4 py-2 text-xs font-bold uppercase tracking-[0.15em] transition-all ${isActiveZone ? "bg-brand text-white shadow-[0_0_16px_rgba(255,106,0,0.4)]" : "border border-white/10 text-white/50 hover:border-brand/50 hover:text-white"}`}>{zone.name}</button>;
            })}
          </div>

          {samplePc && <motion.div key={activeZone} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="cyber-panel p-6">
            <div className="flex flex-col gap-6 md:flex-row md:items-center">
              <div className="flex items-center gap-4"><div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center border-2 border-brand bg-brand/10"><span className="text-[10px] uppercase tracking-widest text-white/50">{zonePrice} BYN</span></div><div><p className="font-display text-xl font-bold text-white">Зона {activeZone}</p></div></div>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {active.zones.slice(0, 3).map((zone, i) => <motion.div key={zone.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="cyber-panel p-5"><div className="flex items-center justify-between"><h4 className="font-display font-bold tracking-widest text-white">{zone.name}</h4><p className="font-display text-xl font-bold text-brand">{zone.pricePerHour} BYN<span className="text-xs text-white/40">/час</span></p></div></motion.div>)}
          </div>
        </motion.div>

        <motion.div key={active.slug + "-map"} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="mx-auto mt-12 max-w-5xl">
          <h2 className="mb-6 font-display text-xl font-bold tracking-widest text-white">СХЕМА ЗАЛА — {active.name.toUpperCase()}</h2>
...
function SpecRow({ label, value }: { label: string; value: string }) {
  return <div><p className="text-[10px] uppercase tracking-widest text-white/30">{label}</p><p className="text-white/80">{value}</p></div>;
}
