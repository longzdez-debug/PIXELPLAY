      ...
      <div className="mx-auto max-w-7xl px-4 pb-24 pt-28 sm:px-6 md:px-10 md:pt-32">
        ...
        >
          {active.features.map((f) => (
            <span key={f} className="cyber-tag">{f}</span>
          ))}
          <span className="cyber-tag !border-white/15 !bg-white/[0.03] !text-white/55">☎ {active.phone}</span>
        </motion.div>
            ...
            {active.zones.map((zone) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  aria-pressed={isActiveZone}
                  className={`cyber-button ${!isActiveZone ? "cyber-button-secondary" : ""} min-h-11 !px-4 !py-2 text-xs !tracking-[0.15em]`}
                >
                  {zone.name}
                </button>
              );
            ...
            >
              <div className="flex flex-col gap-6 md:flex-row md:items-center">
                <div className="flex items-center gap-4">
                  <div className="cyber-tag flex h-16 w-16 shrink-0 flex-col items-center justify-center !border-2 !border-brand !bg-brand/10 !p-0 !text-white/50">
                    <span className="text-[10px] uppercase tracking-widest">{zonePrice} BYN</span>
                  </div>
                  <div>
