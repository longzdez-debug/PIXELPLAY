  ...
  return (
    <main className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-24 pt-28 sm:px-6 md:px-10 md:pt-32">
        ...
        <div className="page-reveal page-reveal-delay-1 mb-10 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {CLUBS.map((club) => {
            const isActive = club.slug === activeSlug;
            return (
              <button key={club.slug} type="button" onClick={() => { setActiveSlug(club.slug); setActiveZone(club.zones[0].name); }} aria-pressed={isActive} className={`cyber-panel group p-5 text-left transition-transform duration-300 hover:-translate-y-1 ${isActive ? "!border-brand shadow-[0_0_24px_rgba(255,106,0,0.2)]" : ""}`}>
                <div className="relative mb-4 h-48 overflow-hidden rounded-lg border border-white/10">
        ...
        <div key={active.slug} className="mb-8 flex flex-wrap gap-2 page-reveal">
        ...
        <div key={`${active.slug}-map`} className="mx-auto mt-12 max-w-5xl page-reveal page-reveal-delay-2">
