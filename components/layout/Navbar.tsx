...
const NAV_ITEMS = [
  { href: "/clubs", label: "Локации" }, { href: "/specs", label: "Железо" },
  { href: "/games", label: "Список игр" }, { href: "/services", label: "Услуги" },
  { href: "/partners", label: "Партнёрам" }, { href: "/pricing", label: "Цены" },
  { href: "/promos", label: "Акции" }, { href: "/tournaments", label: "Турниры" },
];

const MOBILE_NAV_ITEMS = [
  { href: "/services", label: "Услуги" }, { href: "/promos", label: "Акции" },
  { href: "/tournaments", label: "ТУРНИРЫ" }, { href: "/specs", label: "Железо" },
  { href: "/games", label: "Список игр" }, { href: "/partners", label: "Партнёрам" },
...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      if (frame) return;
      frame = window.requestAnimationFrame(() => { setScrolled(window.scrollY > 24); frame = 0; });
    };
    onScroll();
  ...
  useEffect(() => {
    ...
    const showScrollbar = () => {
      ...
      hideTimer = setTimeout(() => document.documentElement.classList.remove("is-scrolling"), 700);
    };
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") { closeMobileMenu(); menuButtonRef.current?.focus(); return; }
      if (event.key !== "Tab" || !mobileNavigationRef.current) return;
      const focusable = Array.from(mobileNavigationRef.current.querySelectorAll<HTMLElement>('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])'));
      ...
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? "bg-void/85 backdrop-blur-md border-b border-brand/15 py-3" : "bg-transparent py-5"}`}>
        <div className="relative flex items-center px-4 md:px-8">
          ...
          <nav className="pointer-events-none absolute inset-x-0 hidden h-8 xl:block">
            <div className="pointer-events-auto absolute right-[calc(50%+86px)] top-0 flex items-center gap-2">{NAV_ITEMS.slice(0, 4).map((item) => <NavLink key={item.href} item={item} pathname={pathname} />)}</div>
            <div className="pointer-events-auto absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"><LetsPlayShowcase /></div>
            <div className="pointer-events-auto absolute left-[calc(50%+86px)] top-0 flex items-center gap-2">{NAV_ITEMS.slice(4).map((item) => <NavLink key={item.href} item={item} pathname={pathname} />)}</div>
          </nav>
          <button type="button" ref={menuButtonRef} onClick={() => setMobileMenuOpen((v) => !v)} className="ml-auto flex h-10 w-10 flex-col items-center justify-center gap-1.5 xl:hidden" aria-label="Меню" aria-expanded={mobileMenuOpen} aria-controls="mobile-navigation">
            <span className={`h-0.5 w-6 bg-brand transition-transform duration-200 ${mobileMenuOpen ? "translate-y-2 rotate-45" : ""}`} /><span className={`h-0.5 w-6 bg-white transition-opacity duration-200 ${mobileMenuOpen ? "opacity-0" : ""}`} /><span className={`h-0.5 w-6 bg-brand transition-transform duration-200 ${mobileMenuOpen ? "-translate-y-2 -rotate-45" : ""}`} />
          </button>
        </div>
        <div className="fixed left-4 top-[50vh] z-[60] hidden -translate-y-1/2 flex-col items-center gap-3 xl:flex">
          <span className="rotate-180 font-display text-[10px] font-bold uppercase tracking-[0.28em] text-white/55 [writing-mode:vertical-rl]">Соцсети</span>
          <div className="flex flex-col items-center gap-3">{SOCIALS.map((s) => <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} title={s.label} className={`flex h-10 w-10 items-center justify-center transition-all group ${s.hover}`}><Image src={`/${s.file}.svg`} alt={s.label} width={22} height={22} style={{ filter: s.filter }} className="h-[22px] w-[22px] object-contain opacity-90 group-hover:opacity-100" /></a>)}</div>
        </div>
...
function NavLink({ item, pathname, firstRef, onClick, mobile = false }: { item: { href: string; label: string }; pathname: string; firstRef?: React.RefObject<HTMLAnchorElement | null>; onClick?: () => void; mobile?: boolean }) {
  ...
  return <Link ref={firstRef} href={item.href} onClick={onClick} className={className}><span className={mobile ? "relative z-10 block font-display text-xs font-black uppercase tracking-[0.18em] text-white/90 transition-all group-hover:text-brand" : "pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-60"}>{mobile ? item.label : ""}</span>{!mobile && item.label}</Link>;
}
