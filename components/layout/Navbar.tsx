    ...
    <>
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? "bg-void/85 backdrop-blur-md border-b border-brand/15 py-3" : "bg-transparent py-5"}`}>
        <div className="relative flex items-center px-4 md:px-8">
          ...
          <nav className="pointer-events-none absolute inset-x-0 hidden h-8 xl:block">
            <div className="pointer-events-auto absolute right-[calc(50%+86px)] top-0 flex items-center gap-2">
              {NAV_ITEMS.slice(0, 4).map((item) => {
                ...
                return <Link key={item.href} href={item.href} className={`group relative flex min-w-[80px] items-center justify-center overflow-hidden whitespace-nowrap rounded-md border px-2 py-1.5 text-[10px] uppercase tracking-[0.14em] ${active ? "border-brand/70 bg-gradient-to-b from-brand/40 to-brand/15 text-brand shadow-[0_0_16px_rgba(255,106,0,0.35)]" : "border-white/10 bg-gradient-to-b from-white/[0.09] via-white/[0.04] to-brand/[0.06] text-white/90 hover:border-brand/40 hover:from-brand/20 hover:to-brand/[0.08] hover:text-white hover:shadow-[0_0_14px_rgba(255,106,0,0.2)]"}`}><span className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-60" />{item.label}</Link>;
              })}
            </div>
          ...
          <button type="button" ref={menuButtonRef} onClick={toggleMobileMenu} className="ml-auto flex h-10 w-10 flex-col items-center justify-center gap-1.5 xl:hidden" aria-label="Меню" aria-expanded={mobileMenuOpen} aria-controls="mobile-navigation">
            <span className={`h-0.5 w-6 bg-brand transition-transform duration-200 ${mobileMenuOpen ? "translate-y-2 rotate-45" : ""}`} />
            <span className={`h-0.5 w-6 bg-white transition-opacity duration-200 ${mobileMenuOpen ? "opacity-0" : ""}`} />
            <span className={`h-0.5 w-6 bg-brand transition-transform duration-200 ${mobileMenuOpen ? "-translate-y-2 -rotate-45" : ""}`} />
          </button>
        </div>
        ...
        <div className="fixed left-4 top-[50vh] z-[60] hidden -translate-y-1/2 flex-col items-center gap-3 xl:flex">
          ...
          <div className="flex flex-col items-center gap-3">
            {SOCIALS.map((s) => <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} title={s.label} className={`flex h-10 w-10 items-center justify-center transition-all group ${s.hover}`}><Image src={`/${s.file}.svg`} alt={s.label} width={22} height={22} style={{ filter: s.filter }} className="h-[22px] w-[22px] object-contain opacity-90 group-hover:opacity-100" /></a>)}
          </div>
        </div>
        ...
        <nav ref={mobileNavigationRef} id="mobile-navigation" aria-label="Мобильная навигация" aria-hidden={!mobileMenuOpen} className={`overflow-hidden rounded-b-3xl border-x border-b border-brand/20 bg-[#090b10]/[0.97] shadow-[0_24px_60px_rgba(0,0,0,0.55),inset_0_1px_0_rgba(255,106,0,0.12)] backdrop-blur-xl transition-[opacity,max-height,transform] duration-200 xl:hidden ${mobileMenuOpen ? "max-h-[90dvh] translate-y-0 opacity-100" : "pointer-events-none max-h-0 -translate-y-2 opacity-0"}`}>
          <div className="flex flex-col gap-2 px-4 py-5 sm:px-6">
            <Link ref={firstMenuLinkRef} href="/services" onClick={() => { closeMobileMenu(); menuButtonRef.current?.focus(); }} className={`group relative mb-2 flex min-h-12 items-center justify-center overflow-hidden rounded-xl border px-3 py-3 text-center transition-all ${isActive(pathname, "/services") ? "border-brand/70 bg-brand/10 text-brand shadow-[0_0_20px_rgba(255,106,0,0.12)]" : "border-white/10 bg-white/[0.025] text-white/90 hover:border-brand/45 hover:bg-brand/5"}`}><span className="relative z-10 block font-display text-xs font-black uppercase tracking-[0.18em] text-white/90 transition-all group-hover:text-brand">Услуги</span></Link>
            ...
            {MOBILE_NAV_ITEMS.slice(1).map((item) => <Link key={item.href} href={item.href} onClick={() => { closeMobileMenu(); menuButtonRef.current?.focus(); }} className={`group relative flex min-h-12 items-center justify-center overflow-hidden rounded-xl border px-3 py-3 text-center transition-all ${isActive(pathname, item.href) ? "border-brand/70 bg-brand/10 text-brand shadow-[0_0_20px_rgba(255,106,0,0.12)]" : "border-white/10 bg-white/[0.025] text-white/90 hover:border-brand/45 hover:bg-brand/5"}`}><span className="relative z-10 block font-display text-xs font-black uppercase tracking-[0.18em] text-white/90 transition-all group-hover:text-brand">{item.label}</span></Link>)}
